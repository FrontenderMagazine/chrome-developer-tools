...
Для создания изначально свёрнутой групы используйте [console.groupCollapsed()][30]
...
Как проиллюстрированно в скриншоте, вызов `timeStamp()` отмечен в следующих
местах:
...
свойств, вы можете выбирать между ними нажимая Tab. Нажав стрелку вправов вы
...
Наконец  метод [x()][55] принимает XPath-путь в качестве параметра и возвращает
...
качестве параметра и отображает элемент или объект в соотвуствующей панеле: DOM-
...
на элемент `<li>`. Затем последнее исполненое выражение ([$_][57]) передаётся в
...
зажатой клавишей Control в консоле и выбрать пункт «Reveal in Elements Panel»
...
Что бы узнать какие типы событий поддерживаются просмотрите [monitorEvents()][64]
...
групируются под одним заголовком:
...
вложены друг в друга. Например, этот пример будет работать так же, как и
предыдущий:
